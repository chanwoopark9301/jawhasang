...
function investmentTextHasConcreteTradeIntent(text) {
  const ask = String(text || '');
  return /(?:[\d,.]+\s*(?:\uC8FC|\uAC1C|shares?|ea)|\$\s*[\d,.]+|[\d,.]+\s*(?:\uB2EC\uB7EC|usd|\uC6D0|\uB9CC\uC6D0|\uC5B5\uC6D0)|\uC804\uB7C9|\uC804\uBD80|\uC808\uBC18|\b(?:INTC|CRCL|ETH-USD|IREN|QLD|QQQM|QQQ|NVDA|AMD|AVGO|TSM|SMH|SOXX)\b)/i.test(ask);
}
...
function shouldRunInvestmentChatTradeGate(text) {
  if (isInvestmentMarketAnalysisIntent(text) && !investmentTextHasConcreteTradeIntent(text)) return false;
...
function _buildChatSysPrompt(isMyRecords, topic, student, extraContext = '', userText = '', chatPlan = null) {
  ...
  if (state.view === 'investment') {
    ...
    const questionIntentBrief = buildInvestmentQuestionIntentBrief(userText);
