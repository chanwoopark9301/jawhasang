...
function normalizeInvestmentState(investment) {
  ...
  return {
    ...
    chatSessions: Array.isArray(src.chatSessions) ? src.chatSessions.map(s => ({
      id: s && s.id ? String(s.id) : '',
      date: s && s.date ? String(s.date) : '',
      market: s && s.market ? String(s.market) : 'US',
      segment: s && s.segment ? String(s.segment) : '',
      label: s && s.label ? String(s.label) : '',
      startedAt: s && s.startedAt ? String(s.startedAt) : null,
      updatedAt: s && s.updatedAt ? String(s.updatedAt) : null,
      closedAt: s && s.closedAt ? String(s.closedAt) : null,
      summarizedAt: s && s.summarizedAt ? String(s.summarizedAt) : null,
      summaryEventId: s && s.summaryEventId ? String(s.summaryEventId) : null,
      summary: s && s.summary ? String(s.summary) : '',
      messageCount: s && s.messageCount ? parseInvestmentNumber(s.messageCount) : 0,
      messages: Array.isArray(s && s.messages) ? s.messages : [],
    })).filter(s => s.id && s.date) : [],
    activeChatSessionId: src.activeChatSessionId ? String(src.activeChatSessionId) : null,
