...
class TestServiceWorker:
    ...
    def test_sw_cache_version_updated_for_reply_mode_ui(self):
        ...
        sw = self._sw()
        assert "CACHE_NAME = 'jip-v48'" in sw, "서비스워커 캐시 버전이 최신이 아님"
...
class TestIndexHTML:
    ...
    def test_static_assets_have_cache_busting_version(self):
        ...
        html = read_html()
        assert 'style.css?v=20260505-01' in html, "CSS 캐시 버스터 없음"
        assert 'js/data.js?v=20260505-01' in html, "data.js 캐시 버스터 없음"
        assert 'js/chat.js?v=20260505-01' in html, "chat.js 캐시 버스터 없음"
        assert 'js/investment-api.js?v=20260505-01' in html, "investment-api.js 캐시 버스터 없음"
