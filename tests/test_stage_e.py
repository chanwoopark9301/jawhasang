...
class TestServiceWorker:
    ...
    def test_sw_cache_version_updated_for_reply_mode_ui(self):
        ...
        sw = self._sw()
        assert "CACHE_NAME = 'jip-v169'" in sw, "서비스워커 캐시 버전이 최신이 아님"
...
class TestIndexHTML:
    ...
    def test_static_assets_have_cache_busting_version(self):
        ...
        html = read_html()
        assert 'style.css?v=20260514-13' in html, "CSS 캐시 버스터 없음"
        assert 'js/data.js?v=20260514-13' in html, "data.js 캐시 버스터 없음"
        assert 'js/chat.js?v=20260514-13' in html, "chat.js 캐시 버스터 없음"
        assert 'js/reminders.js?v=20260514-13' in html, "reminders.js 캐시 버스터 없음"
        assert 'js/investment-ledger-engine.js?v=20260514-13' in html, "investment-ledger-engine.js 캐시 버스터 없음"
        assert 'js/investment-format.js?v=20260514-13' in html, "investment-format.js 캐시 버스터 없음"
        assert 'js/investment-portfolio.js?v=20260514-13' in html, "investment-portfolio.js 캐시 버스터 없음"
        assert 'js/investment-desk.js?v=20260514-13' in html, "investment-desk.js 캐시 버스터 없음"
        assert 'js/investment-actions.js?v=20260514-13' in html, "investment-actions.js 캐시 버스터 없음"
        assert 'js/investment-api.js?v=20260514-13' in html, "investment-api.js 캐시 버스터 없음"
...
class TestInvestmentToastLocalizationAndSpeed:
    ...
    def test_investment_chat_classifies_market_intent_before_controls(self):
        chat = read_file('js/chat.js')
        plan = read_file('BRIEFING_QUALITY_UPGRADE.md')
        assert 'classifyInvestmentQuestionIntent' in chat
        assert 'buildInvestmentQuestionIntentBrief' in chat
        assert 'market_regime' in chat
        assert 'sector_rotation' in chat
        assert 'entry_timing' in chat
        assert 'market_psychology' in chat
        assert 'Interpretation comes before behavior control' in chat
        assert 'market participant psychology' in chat
