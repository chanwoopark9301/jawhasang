...
- 자유 대화창의 매수/추매/매도/손절/보유 의도를 Python 엔진이 감지하고 `/api/investment/chat-gate`로 AI 호출 전에 먼저 평가한다.
- 대화 게이트 결과는 투자 결정 로그에 `chat-trade-gate`로 남기되, 포트폴리오 원장에는 자동 반영하지 않는다.
